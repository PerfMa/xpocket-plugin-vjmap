...
import com.perfma.xpocket.plugin.vjtools.vjmap.utils.ProgressNotifier;
...
public class OldgenAccessor {
	...
	public void caculateHistogram(XPocketProcess tty) {
		...
		for (; cur.lessThan(limit);) {
			...
			if (FreeChunk.indicatesFreeChunk(cur)) {
				...
				if (progressNotifier.processingSize > progressNotifier.nextNotificationSize) {
					progressNotifier.printProgress(tty);
				}
	...
	private void printGenSummary(ConcurrentMarkSweepGeneration cmsGen, XPocketProcess tty) {
